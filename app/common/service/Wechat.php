...
class Wechat
{
    ...
    public function __construct($token = '', $app_id = '', $app_secret = '',$encodingAesKey = '')
    {
        $this->token = !empty($token) ? $token : Config::get('weixin.token');
        $this->appid = !empty($app_id) ? $app_id : Config::get('weixin.app_id');
        $this->appsecret = !empty($app_secret) ? $app_secret : Config::get('weixin.app_secret');
    ...
    public function reply($msg=array(),$return = false)
    {
        ...
        $this->log($xmldata);
        if (!empty($this->encrypt_type) && $this->encrypt_type == 'aes') { //如果来源消息为加密方式
            $pc = new Prpcrypt($this->encodingAesKey);
