...
class Cart extends Weixin
{
    ...
    public function index(){
        ...
        $products = $cartModel->getList($this->member_id);
        if(!empty($products)){
    ...
    public function add($product_id,$num){
        ...
        $cart = Db::name('cart')->where('member_id',$this->member_id)->where('product_id',$product_id)->find();
    ...
    public function update($product_id,$num){
        ...
        if($store_nums <= 0 || ($cart && $num > $store_nums)){
            ...
            Db::name('cart')->where('member_id',$this->member_id)->where('product_id',$product_id)->setField('cart_num',$num);
            return json(['code'=>-2,'msg'=>"库存不足，最多购买{$store_nums}件",'num' => $num]);
        }
        if(!empty($cart)){
            $result = Db::name('cart')->where('member_id',$this->member_id)->where('product_id',$product_id)->setField('cart_num',$num);
        }
    ...
    public function setSelected($cart_id,$is_selected){
        if(empty($cart_id)){
            Db::name('cart')->where('member_id',$this->member_id)->setField('is_selected',boolval($is_selected));
            return json(['code'=>1,'msg'=>'更新成功']);
        }else{
            $cart = Db::name('cart')->where('member_id',$this->member_id)->where('id',$cart_id)->find();
            if(!empty($cart)){
                Db::name('cart')->where('member_id',$this->member_id)->where('id',$cart_id)->setField('is_selected',boolval($is_selected));
                return json(['code'=>1,'msg'=>'更新成功']);
            }else{
